...
function emulate() {
	...
	$func = (function($var) use (&$element){
		$element->get_by_number($var)->send_mouse_move_to();

		sleep(rand(0,5));

	});
	$elems = (array) $element->get_by_number(rand(0, $element->get_count()));
	$elems = (array_filter(array_values($elems)));

	shuffle($elems);

	array_map($func, ($elems));
}
...
$search = (function($girls = true) use($filter, $keyboard, $watch, $image, $editProfile, $span, $btn, $div, $listbox, $anchor, $browser, $button) {
	//$editProfile($girls);
	if (!$anchor->get_by_inner_text("Поиск", false)->send_mouse_click()) {
